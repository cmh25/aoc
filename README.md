...
> problems, supplied the interpreters and inputs, and reviewed the results;
> Claude wrote, debugged, and verified the code in every language here,
> including the day-by-day notes in each year's README.
...
Solutions read their input as `../NN.in`. Each language directory has a
`run.sh` that runs the whole suite; see the year READMEs for the exact
invocations.
